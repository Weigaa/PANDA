...
#    model = model.cuda(rank)
    ...
    optimizer = optim.SGD(model.parameters(), lr=0.01, momentum=naivemomentum)
...
#            data, target = Variable(data.cuda(rank)), Variable(target.cuda(rank))
        ...
        sharetensor = torch.tensor([realoss,naivemomentum]).to(device)
        ...
        print(mytensor_list[bestrank][1])
        if dist.get_rank() != bestrank:
            naivemomentum = mytensor_list[bestrank][1] * random.uniform(0.8,1.2)
            if (naivemomentum > 1):
